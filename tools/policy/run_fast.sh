...
say(){ [[ "$QUIET" == "1" ]] || echo "$@"; }
say "== bootstrap =="tools/policy/bootstrap_check.sh || truetools/policy/bootstrap_policy.sh || true
if [[ "$VERIFY_ONLY" == "1" ]]; then
  EVAL=0; PUBLISH=0
  say "== verify-only mode =="fi
if [[ "$EVAL" == "1" ]]; then
  say "== eval/propose =="  (python3 tools/policy/evaluate_and_propose.py >/dev/null 2>&1) || python3 tools/policy/evaluate_and_propose.py || true
fi
if [[ "$PUBLISH" == "1" ]]; then
  say "== publish/sign =="  KID="$KID" tools/policy/publish_policy.sh
fi
if [[ "$VERIFY" == "1" ]]; then
  say "== verify signatures =="  if [[ -n "$PUB" ]]; then
